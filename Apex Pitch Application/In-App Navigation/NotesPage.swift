        ...
        NavigationStack {
            List {
                ...
                if store.notes.isEmpty {
                    ContentUnavailableView(
                        ...
                        description: Text("Tap the + button to save feedback.")
                    )
                } else {
                    ...
                    ForEach(store.notes) { note in
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text(note.feedback).font(.body).lineLimit(3)
                            
                            if !note.actionItems.isEmpty {
                                VStack(alignment: .leading, spacing: 4) {
                                    Text("Action Items").font(.caption).fontWeight(.semibold)
                                    Text(note.actionItems).font(.caption)
                                }
                            }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button { showingAddNote = true } label: { Image(systemName: "plus") }
                }
            }
            ...
            .task {
                store.subscribeToNotes()
                
                print("🚀 Fetching notes now...")
                await store.fetchNotes()
            }
        }
    }
